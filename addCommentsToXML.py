...
#    for track in summaryTracks.values(): track.write(sys.stdout, False)
    ...
    for fname in fnames:
        ...
        if startTime in summaryTracks:
            if summaryTracks[startTime].comment != "": tree.addElem("", "Comment", summaryTracks[startTime].comment)
            if summaryTracks[startTime].course != "": tree.addElem("", "Course", summaryTracks[startTime].course)
            tree.write(fname)
